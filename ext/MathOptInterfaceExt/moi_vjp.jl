function _compute_qp_vjp_cons!(grad_p, f::MOI.ScalarAffineFunction, ctx, x, dλ_row, dx, scale)
    for term in f.terms
        i = get_param_idx(ctx, term.variable)
        !iszero(i) && (grad_p[i] += term.coefficient * dλ_row)
    end
    return
end
function _compute_qp_vjp_cons!(grad_p, f::MOI.ScalarQuadraticFunction, ctx, x, dλ_row, dx, scale)
    p_vals = ctx.p_vals
    ...
    for term in f.quadratic_terms
        ...
        !iszero(pi) && !iszero(xj) && (grad_p[pi] += coef * x[xj] * dλ_row)
        !iszero(pj) && !iszero(xi) && (grad_p[pj] += coef * x[xi] * dλ_row)
        !iszero(pi) && !iszero(xj) && (grad_p[pi] += scale * coef * dx[xj])
        !iszero(pj) && !iszero(xi) && (grad_p[pj] += scale * coef * dx[xi])
        if !iszero(pi) && !iszero(pj)
            if pi == pj
                grad_p[pi] += coef * p_vals[pi] * dλ_row
            else
                grad_p[pi] += coef * p_vals[pj] * dλ_row
                grad_p[pj] += coef * p_vals[pi] * dλ_row
            end
        end
    end
    return
...
function _compute_nlp_vjp_objcons!(evaluator, ctx, grad_p, x_combined::AbstractVector{T}, y, dx, dλ, n_x, σ) where {T}
    n_p = length(grad_p)
    n_nlp = length(dλ)

    if n_nlp > 0
        hvp_result = ctx.hvp_result
        MOI.eval_constraint_jacobian_transpose_product(evaluator, hvp_result, x_combined, dλ)
...
function _compute_param_pullback!(model, x::AbstractVector{T}, y, dx, dλ, ctx::SensitivityContext) where {T}
    grad_p = ctx.grad_p
    fill!(grad_p, zero(T))
    ...
    for (row, constraint) in enumerate(model.qp_data.constraints)
        _compute_qp_vjp_cons!(grad_p, constraint, ctx, x_dense, dλ[row], dx, y[row])
    end
    ...
    if has_nlp && !isnothing(model.nlp_model)
        ...
        dμ = @view dλ[(ctx.n_qp + 1):(ctx.n_qp + ctx.n_nlp)]
